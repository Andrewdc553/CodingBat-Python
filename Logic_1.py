...
date_fashion(5, 5) → 1
"""
## Check the <=2 case first, since it takes precedence over the >=8 case.
  if you <= 2 or date <=2:
    return 0
...
"""
 weekdays_ring = "7:00" if not vacation else "10:00"
 weekend_ring = "10:00" if not vacation else "off"
 return weekend_ring if day in [0, 6] else weekdays_ring 
 
 # nv vs v
 1-5: weekdays 7 vs 10
 6-0: weekend 10 vs off 
...
a multiple of 10. Note: (a % b) is the remainder of dividing a by b, so (7 % 5) is 2.
"""
  a = num % 10      
  return 2 >= a or 8 <= a 
